...
local _exportFrame, _importFrame, _fluencyFrame  -- lazily created
...
-- Fluency picker shown when learning a language from the Languages panel.
local function GetFluencyFrame()
    if _fluencyFrame then return _fluencyFrame end

    local f = MakeCodeDialog("SpeakethLearnLanguageFrame", 390, 190)

    local title = f:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    title:SetPoint("TOPLEFT", f, "TOPLEFT", 16, -14)
    title:SetText("Learn Language")
    title:SetTextColor(1, 1, 1, 1)
    f._title = title
    ...
    desc:SetPoint("RIGHT", f, "RIGHT", -16, 0)
    desc:SetJustifyH("LEFT")
    desc:SetText("Choose the character's starting fluency.")

    local valueText = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    valueText:SetPoint("TOP", desc, "BOTTOM", 0, -14)
    valueText:SetText("100% fluency")
    f._valueText = valueText

    local slider = CreateFrame("Slider", "SpeakethLearnLanguageSlider", f, "OptionsSliderTemplate")
    slider:SetPoint("TOPLEFT", desc, "BOTTOMLEFT", 12, -40)
    slider:SetPoint("RIGHT", f, "RIGHT", -28, 0)
    slider:SetHeight(20)
    ...
    slider:SetScript("OnValueChanged", function(_, value)
        valueText:SetText(string.format("%d%% fluency", math.floor(value + 0.5)))
    end)
    f._slider = slider

    local learnBtn = CreateFrame("Button", nil, f, "UIPanelButtonTemplate")
    learnBtn:SetSize(84, 24)
    learnBtn:SetPoint("BOTTOMRIGHT", f, "BOTTOM", -4, 12)
    learnBtn:SetText("Learn")
    learnBtn:SetScript("OnClick", function()
        local value = math.floor(f._slider:GetValue() + 0.5)
...
function Speaketh_Options:PromptLearnLanguage(langName, onAccept)
    local f = GetFluencyFrame()
    f._title:SetText("Learn " .. (langName or "Language"))
    f._onAccept = onAccept
    f._slider:SetValue(100)
    f._valueText:SetText("100% fluency")
    f:Show()
end
...
local function BuildGeneralPanel(panel)
    ...
    local glyphCB = MakeCheck(body,
        "Show language glyphs in speech bubbles",
        "When enabled, overhead speech bubbles use glyph textures for languages that currently ship a dedicated glyph set. Unsupported languages keep their normal scrambled text.\n\nThe chat window is unaffected: you still read messages translated according to your fluency. Players without Speaketh always see normal text.\n\n(Chat bubbles are unavailable inside dungeons and raids.)",
        function() return SV().showGlyphs == true end,
        function(v) SetSV("showGlyphs", v) end)
    glyphCB:SetPoint("TOPLEFT", hudCB, "BOTTOMLEFT", 0, -4)
    ...
    splashCB:SetPoint("TOPLEFT", glyphCB, "BOTTOMLEFT", 0, -4)
...
-- Effects
-- ============================================================
local function BuildEffectsPanel(panel)
    local body = panel.body

    local desc = body:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    desc:SetPoint("TOPLEFT", body, "TOPLEFT", 0, 0)
    desc:SetPoint("RIGHT", body, "RIGHT", 0, 0)
    desc:SetJustifyH("LEFT")
    desc:SetText("Choose one speech effect. Effects operate independently from your selected dialect. Stutter, Hiss, Growl, and Lisp only alter spoken words; Drunk keeps its existing hiccups.")
    Speaketh_Theme:Register(function(C)
        local t = C.bodyText; desc:SetTextColor(t[1], t[2], t[3], 1)
    end)

    local chooseHeader = DrawSectionHeader(body, desc, "BOTTOMLEFT", 0, -20, "Active Effect")
    local effectButtons = {}
    local _, effectOrder = Speaketh_Dialects:GetEffects()
    local choices = {false}
    for _, key in ipairs(effectOrder) do table.insert(choices, key) end

    local RefreshButtons
    for i, key in ipairs(choices) do
        local effectKey = key
        local label = effectKey and (Speaketh_Dialects:GetData(effectKey).name or effectKey) or "None"
        local btn = GoldBtn(body, label, 96, 24)
        local col = (i - 1) % 3
        local row = math.floor((i - 1) / 3)
        btn:SetPoint("TOPLEFT", chooseHeader, "BOTTOMLEFT", col * 106, -8 - row * 32)
        btn:SetScript("OnClick", function()
            Speaketh_Dialects:SetActiveEffect(effectKey or nil)
            RefreshButtons()
            if Speaketh_UI and Speaketh_UI.RefreshWindow then Speaketh_UI:RefreshWindow() end
        end)
        effectButtons[effectKey or "None"] = btn
    end

    local levelLabel = body:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    levelLabel:SetPoint("TOPLEFT", chooseHeader, "BOTTOMLEFT", 4, -84)
    levelLabel:SetText("Effect Intensity")

    local slider = CreateFrame("Slider", "SpeakethOptionsEffectDrunkSlider", body,
        "OptionsSliderTemplate")
    slider:SetPoint("TOPLEFT", levelLabel, "BOTTOMLEFT", 12, -10)
    slider:SetSize(320, 20)
    slider:SetMinMaxValues(0, 3)
    slider:SetValueStep(1)
    slider:SetObeyStepOnDrag(true)
    slider.Low:SetText("Off")
    slider.High:SetText("Strong")
    slider:SetScript("OnValueChanged", function(self, value)
        if panel._syncingEffectSlider then return end
        value = math.floor(value + 0.5)
        local active = Speaketh_Dialects:GetActiveEffect()
        if active then Speaketh_Dialects:SetLevel(active, value) end
        local label = active and Speaketh_Dialects:GetSliderLabel(active, value) or "Off"
    ...
    RefreshButtons = function()
        local active = Speaketh_Dialects:GetActiveEffect()
        local acc, hi, mut = Speaketh_Theme.C.accent,
            Speaketh_Theme.C.btnTextHover, Speaketh_Theme.C.btnText
        for key, btn in pairs(effectButtons) do
            local selected = (key == "None" and active == nil) or key == active
            btn._bg:SetColorTexture(acc[1], acc[2], acc[3], selected and 0.30 or 0.10)
            btn.Text:SetTextColor(
                selected and hi[1] or mut[1],
                selected and hi[2] or mut[2],
                selected and hi[3] or mut[3], 1)
        end
        local data = active and Speaketh_Dialects:GetData(active)
        local level = active and Speaketh_Dialects:GetLevel(active) or 0
        panel._syncingEffectSlider = true
        slider:SetValue(level)
        panel._syncingEffectSlider = false
        if slider.Text then
            slider.Text:SetText(active and Speaketh_Dialects:GetSliderLabel(active, level) or "Off")
        end
        if data and data.usesSlider then
            local labels = data.sliderLabels or {"Off", "Light", "Moderate", "Strong"}
            levelLabel:SetText((data.name or active) .. " Intensity")
            slider.Low:SetText(labels[1] or "Off")
            slider.High:SetText(labels[#labels] or "Strong")
            levelLabel:Show(); slider:Show()
        else
            levelLabel:Hide(); slider:Hide()
        end
    end
    Speaketh_Theme:Register(RefreshButtons)
    panel.refresh = RefreshButtons
end
...
local function BuildNewLanguagePanel(panel)
    ...
    createBtn:SetScript("OnClick", function()
        ...
        if editingKey then
            ...
            SetStatus('"'..name..'" added ('..#wordList..' words). Learn it from the Languages panel to start speaking it.', false)
            Rebuild()
        end
    end)
...
-- Category: Languages
-- ============================================================
local function BuildLanguagesPanel(panel)
    local body = panel.body
    ...
    local function RefreshLanguageDisplays()
        if Speaketh_UI and Speaketh_UI.RefreshWindow then
            Speaketh_UI:RefreshWindow()
        end
        if Speaketh_UI and Speaketh_UI.RefreshLanguageHUD then
    ...
    local function CreateLanguageRow(langKey)
        local row = CreateFrame("Frame", nil, content)
        row:SetHeight(36)
        row.langKey = langKey
        ...
        Speaketh_Theme:Register(function(C)
            local a = C.accent
            bg:SetColorTexture(a[1], a[2], a[3], 0.07)
        end)
        ...
        Speaketh_Theme:Register(function(C)
            local t = C.bodyText
            name:SetTextColor(t[1], t[2], t[3], 1)
        end)

        local action = GoldBtn(row, "Learn", 76, 20)
        action:SetPoint("RIGHT", row, "RIGHT", -6, 0)
        row.action = action

        local status = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        status:SetPoint("RIGHT", action, "LEFT", -12, 0)
        status:SetJustifyH("RIGHT")
        row.status = status

        action:SetScript("OnClick", function()
            local key = row.langKey
            local current = Speaketh_Fluency and Speaketh_Fluency:Get(key) or 0
            if current > 0 then
                Speaketh_Fluency:Set(key, 0)
                if Speaketh and Speaketh.GetLanguage and Speaketh:GetLanguage() == key then
                    Speaketh:SetLanguage("None")
                end
                RefreshLanguageDisplays()
                Rebuild()
                return
            end

            local displayName = Speaketh and Speaketh.GetLanguageDisplayName
                and Speaketh:GetLanguageDisplayName(key) or key
            Speaketh_Options:PromptLearnLanguage(displayName, function(value)
                Speaketh_Fluency:Set(key, value)
                RefreshLanguageDisplays()
                Rebuild()
            end)
        end)
    ...
    Rebuild = function()
        ...
        local previous
        local count = 0
        for _, langKey in ipairs(Speaketh_LanguageOrder or {}) do
            if Speaketh_Languages and Speaketh_Languages[langKey] then
                local row = rows[langKey] or CreateLanguageRow(langKey)
                row.langKey = langKey
                row:ClearAllPoints()
                ...
                local fluency = Speaketh_Fluency and Speaketh_Fluency:Get(langKey) or 0
                row.name:SetText(displayName)
                if fluency > 0 then
                    row.status:SetText(string.format("|cff55dd88LEARNED (%d%%)|r", math.floor(fluency)))
                    row.action:SetText("Unlearn")
                else
                    row.status:SetText("|cff888888UNLEARNED|r")
                    row.action:SetText("Learn")
                end
        ...
        content:SetHeight(math.max(1, count * 42 - 6))
    end
...
local function BuildModulesPanel(panel)
    ...
    local function AddModuleRow(displayName, isIntegrated)
        ...
        Speaketh_Theme:Register(function(C)
            local a = C.accent
            bg:SetColorTexture(a[1], a[2], a[3], 0.08)
        end)
        ...
        status:SetPoint("RIGHT", row, "RIGHT", -16, 0)
        status:SetJustifyH("RIGHT")

        local function Refresh()
            local active = isIntegrated() == true
            if active then
                status:SetText("|cff55dd88ACTIVE|r")
                row:Show()
            else
                row:Hide()
            end
    ...
    local chatteryRow, refreshChattery = AddModuleRow(
        "Chattery",
        function()
            return Speaketh and Speaketh.ChatteryCompatibilityActive == true
        end)
    ...
    panel.refresh = function()
        ...
        local anchor
        if refreshChattery() then
            PositionRow(chatteryRow, anchor)
            anchor = chatteryRow
        end
...
local function BuildAboutPanel(panel)
    ...
    local featureList = {
        ...
        "Speech effects - Drunk, Stutter, Hiss, Growl, and Lisp",
    ...
    local commands = {
        ...
        {"/sp dialect <n>",      "Set dialect  (gilnean, troll, etc.)"},
    ...
    --             + gap(14) + cmdHead(14) + 12 commands*15 + bottom pad(20)
    content:SetHeight(16 + 14 + 14 + 10*16 + 14 + 14 + 12*15 + 20)
end
...
function Speaketh_Options:Open()
    ...
    if not _categoryButtons.general then
        ...
        y = y - (ROW_H + ROW_GAP)
        AddCategory("effects", "Effects", BuildEffectsPanel, y)

        y = y - (ROW_H + ROW_GAP)
        AddCategory("languages", "Languages", BuildLanguagesPanel, y)
        ...
        -- Modules and About buttons move when Word Rules expands/collapses.
        local modulesCollapsedY = wrY - (ROW_H + ROW_GAP)
        local aboutCollapsedY   = modulesCollapsedY - (ROW_H + ROW_GAP)
        local modulesExpandedY  = wrY - (ROW_H + SUB_GAP) - SUB_BLOCK - ROW_GAP
        local aboutExpandedY    = modulesExpandedY - (ROW_H + ROW_GAP)
        local modulesBtn, aboutBtn
        ...
        local function SetWordRulesExpanded(expanded)
            ...
            if expanded then
                ...
                modulesBtn:ClearAllPoints()
                modulesBtn:SetPoint("TOPLEFT", _mainFrame, "TOPLEFT", SIDE_INSET, modulesExpandedY)
                ...
                modulesBtn:ClearAllPoints()
                modulesBtn:SetPoint("TOPLEFT", _mainFrame, "TOPLEFT", SIDE_INSET, modulesCollapsedY)
                aboutBtn:ClearAllPoints()
        ...
        -- Modules and About start in their collapsed positions.
        y = modulesCollapsedY
        AddCategory("modules", "Modules", BuildModulesPanel, y)
        modulesBtn = _categoryButtons["modules"]
