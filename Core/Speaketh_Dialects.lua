...
local function RegisterEffect(key, data)
    DIALECTS[key] = data -- reuse the established transformation engine
    EFFECTS[key] = true
    table.insert(EFFECT_ORDER, key)
end
...
-- Speech effects add sounds or word transformations only. They never insert
-- roleplay actions such as *growls* or *hisses*.
local EFFECT_SLIDER_LABELS = {"Off", "Light", "Moderate", "Strong"}
local EFFECT_SLIDER_COLORS = {
    {0.50, 0.50, 0.50},
    {0.70, 0.78, 0.95},
    {0.62, 0.65, 0.92},
    {0.72, 0.48, 0.88},
...
local function StutterWords(text, level)
    local chance = ({15, 30, 50})[level] or 15
    return (text:gsub("([%a])([%a'%-]+)", function(first, rest)
        if #rest < 2 or math.random(1, 100) > chance then
            return first .. rest
        end
...
local function LispWords(text, level)
    local chance = ({25, 50, 75})[level] or 25
    return (text:gsub("[%a'%-]+", function(word)
        if not word:find("[sS]") or math.random(1, 100) > chance then
            return word
        end
        word = word:gsub("S", "Th")
        return word:gsub("s", "th")
    end))
end

RegisterEffect("Stutter", {
    name = "Stutter",
    usesSlider = true,
    sliderLabels = EFFECT_SLIDER_LABELS,
    sliderColors = EFFECT_SLIDER_COLORS,
    slur = StutterWords,
})
...
RegisterEffect("Hiss", {
    ...
    interjectionChance = {[1]=5, [2]=10, [3]=16},
    interjectionEndChance = {[1]=5, [2]=10, [3]=15},
})

RegisterEffect("Growl", {
    name = "Growl",
    usesSlider = true,
    sliderLabels = EFFECT_SLIDER_LABELS,
    sliderColors = EFFECT_SLIDER_COLORS,
    ...
    interjectionChance = {[1]=4, [2]=8, [3]=13},
    interjectionEndChance = {[1]=6, [2]=12, [3]=18},
})
...
function Speaketh_Dialects:GetEffects()
    return DIALECTS, EFFECT_ORDER
end

function Speaketh_Dialects:IsEffect(key)
    return EFFECTS[key] == true
end

function Speaketh_Dialects:GetActive()
    local active = Speaketh_Char and Speaketh_Char.dialect or nil
    return not EFFECTS[active] and active or nil
end
...
function Speaketh_Dialects:SetActive(key)
    ...
    if key then
        ...
        if not Speaketh_Char.dialectLevels[key] then
            Speaketh_Char.dialectLevels[key] = 3
        end
...
function Speaketh_Dialects:GetActiveEffect()
    local active = Speaketh_Char and Speaketh_Char.effect or nil
    return EFFECTS[active] and active or nil
end

function Speaketh_Dialects:SetActiveEffect(key)
    if not Speaketh_Char then return end
    if key and not EFFECTS[key] then
        DEFAULT_CHAT_FRAME:AddMessage(
            "|cffffcc00[Speaketh]|r Unknown effect: " .. tostring(key))
        return
    end
    Speaketh_Char.effect = key
    Speaketh_Char.effectLevels = Speaketh_Char.effectLevels or {}
    if key and DIALECTS[key] and DIALECTS[key].usesSlider
       and (Speaketh_Char.effectLevels[key] or 0) <= 0 then
        Speaketh_Char.effectLevels[key] = 1
    end
end
...
function Speaketh_Dialects:GetLevel(key)
    ...
    if d and d.usesSlider then
        local levels = EFFECTS[key]
            and Speaketh_Char and Speaketh_Char.effectLevels
            or Speaketh_Char and Speaketh_Char.dialectLevels
        local stored = levels and levels[key]
        if stored ~= nil then return stored end
        if EFFECTS[key] then
            -- Older saves stored on/off effects without a level. Preserve an
            -- active effect at the new conservative Light intensity.
            return self:GetActiveEffect() == key and 1 or 0
        end
        return 3
    else
...
-- ============================================================
local function ApplyOne(self, text, langKey, key)
...
function Speaketh_Dialects:Apply(text, langKey)
    text = ApplyOne(self, text, langKey, self:GetActive())
...
-- ============================================================
local function ApplyInterjectionsOne(self, text, key)
    if not key then return text end
    local d = DIALECTS[key]
