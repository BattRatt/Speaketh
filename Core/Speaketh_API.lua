...
-- Stable splitter-facing delegates. The implementation remains in the main
-- file so built-in and third-party send paths share exactly the same behavior.
function API:WouldTranslate(chatType)
    return Speaketh and Speaketh.WouldTranslate
       and Speaketh:WouldTranslate(chatType) or false
end

function API:GetTagOverhead(chatType)
    return Speaketh and Speaketh.GetTagOverhead
       and Speaketh:GetTagOverhead(chatType) or 0
end

function API:TranslateChunk(text, chatType, target)
    if type(text) ~= "string" or text == "" then return text or "" end
    if not self:WouldTranslate(chatType) then return text end
    if not Speaketh or type(Speaketh.TranslateChunk) ~= "function" then return text end
    local ok, translated = pcall(Speaketh.TranslateChunk, Speaketh, text, chatType, target)
...
-- Splitters call this while they own the current edit-box send so Speaketh's
-- built-in callback does not translate the first chunk a second time.
function API:SetSplitterBypassing(active)
    if Speaketh then Speaketh.splitterBypassing = active and true or false end
end

function API:IsSplitterBypassing()
    return Speaketh and Speaketh.splitterBypassing == true or false
end
...
function API:Translate(text, opts)
    ...
    if DIALECT_QUOTES_ONLY[chatType] then
        ...
        local broadcastLang = langKey ~= "None" and langKey or nil
        return final, broadcastLang, "ok"
    end
    ...
    if langKey == "None" then
        ...
        if not dialect and not effect then
            return text, nil, "passthrough"
        end
    end
...
-- message afterwards.
--
-- Intended for listener/snooper/log addons that want to inspect a decoded
-- copy for their own UI. This is non-destructive and does not consume the
-- entry Speaketh's own chat filter still needs.
--
